...
from RX.models import ScientificOfficeStock, ScientificOffice, ScientificOfficeItems, Invoices, DistributorWarehouse, \
    Order
from RX.schemas.SBItemStock import SBStockOut, ScientificOfficeOut, SBInvoiceInBody, SBInvoiceOut, SBOrderOut
...
def add_invoice(request, ListPayload: list[SBInvoiceInBody]):
    print(ListPayload)
    auth_user = User.objects.get(id=request.auth['id'])
